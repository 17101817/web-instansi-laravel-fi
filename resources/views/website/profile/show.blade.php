...
@section('content')
    ...
    <div class="container-fluid page-header-bg py-5 mb-5" style="--page-header-bg: url('{{ $headerImage }}');">
        ...
        <div class="container py-5">
            <h1 class="display-3 text-white mb-3 animated slideInDown">{{ $profile->name }}</h1>
            <nav aria-label="breadcrumb animated slideInDown">
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a class="text-white" href="{{ '/' }}">Home</a></li>
                    <li class="breadcrumb-item"><a class="text-white" href="#">{{ $title }}</a></li>
                    <li class="breadcrumb-item text-white active" aria-current="page">{{ $profile->name }}</li>
                </ol>
            </nav>
        </div>
    </div>
    ...
    <div class="container py-5">
        ...
        @if (is_array($profile->dokumen) && count($profile->dokumen))
            <h5 class="mb-3">Lampiran</h5>

            <div class="row g-4">
                @foreach ($profile->dokumen as $item)
                    @php
                        // 🔒 Normalisasi file
                        $nama = $item['nama'] ?? null;
