...
@section('content')
    ...
    <div class="container-fluid page-header-bg py-5 mb-5" style="--page-header-bg: url('{{ $headerImage }}');">
        ...
        <div class="container py-5">
            <h1 class="display-3 text-white mb-3 animated slideInDown">{{ $Skm->name }}</h1>
            <nav aria-label="breadcrumb animated slideInDown">
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a class="text-white" href="{{ '/' }}">Home</a></li>
                    <li class="breadcrumb-item"><a class="text-white" href="#">{{ $title }}</a></li>
                    <li class="breadcrumb-item text-white active" aria-current="page">{{ $Skm->name }}</li>
                </ol>
            </nav>
        </div>
    </div>
    ...
    <div class="container py-5">
        ...
        <h1 class="mb-4">{{ $Skm->name }}</h1>

        <div class="mb-4">
            {!! $Skm->deskripsi !!}
        </div>

        @if (is_array($Skm->dokumen) && count($Skm->dokumen))
            <h5 class="mb-3">Lampiran</h5>

            <div class="row g-4">
                @foreach ($Skm->dokumen as $item)
                    @php
                        // 🔒 Normalisasi file
                        $nama = $item['nama'] ?? null;
